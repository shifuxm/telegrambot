// backend/src/api/routes/_routes.js
// 7-modul: payment search arxivlanganlarni ham qidiradi
...
attRouter.get('/admin-sheet/:scheduleId', roleCheck('admin', 'receptionist'), async (req, res) => {
  try {
    const scheduleId = parseInt(req.params.scheduleId);
    const schedule = await prisma.schedule.findUnique({
      where: { id: scheduleId },
      include: { group: { include: { subject: true, teacher: { select: { firstName: true, lastName: true } } } } }
    });
    if (!schedule) return res.status(404).json({ error: 'Jadval topilmadi' });

    const today = moment().tz(TZ).startOf('day').toDate();
    if (schedule.lessonDate > today) {
      return res.status(400).json({ error: 'Kelajakdagi dars uchun davomat olib bo\'lmaydi' });
    }
    ...
    res.json({
      scheduleId, lessonDate: schedule.lessonDate,
      startTime: schedule.startTime, endTime: schedule.endTime,
      groupName: schedule.group.name, subjectName: schedule.group.subject.name,
      teacherName: schedule.group.teacher
        ? `${schedule.group.teacher.firstName} ${schedule.group.teacher.lastName}`
        : null,
      alreadyTaken, students
    });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

attRouter.post('/admin-save/:scheduleId', roleCheck('admin', 'receptionist'), async (req, res) => {
  try {
    ...
    const prevAttendances = await prisma.attendance.findMany({
      where: { scheduleId }, select: { groupStudentId: true, isPresent: true }
    });
    ...
    res.json({ success: true, count: groupStudents.length });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
payRouter.get('/', roleCheck('admin', 'receptionist'), async (req, res) => {
  ...
  // auto_deduction lar ko'rsatilmasin
...
// ── YANGILANGAN: arxivlangan o'quvchilar ham chiqadi ─────────────────────────
payRouter.get('/search', roleCheck('admin', 'receptionist'), async (req, res) => {
  ...
  res.json(students.map(s => ({
    ...s,
    isArchived: s.groupStudents.every(gs => gs.status === 'archived'),
    activeGroups: s.groupStudents.filter(gs => gs.status === 'active')
  })));
});
  ...
  try {
    ...
    const [groupsCount, totalStudents, totalStudentsActive, income, expense] = await Promise.all([
      ...
      prisma.payment.findMany({
        where: { monthYear: target, note: { not: 'auto_deduction' } },
    ...
    if (prevMonth >= MIN_MONTH) {
      const [pInc, pExp] = await Promise.all([
        prisma.payment.aggregate({ where: { monthYear: prevMonth, note: { not: 'auto_deduction' } }, _sum: { amount: true } }),
      ...
      include: {
        ...
        student: {
          include: {
            ...
            payments: { where: { monthYear, note: { not: 'auto_deduction' } } }
          }
        }
      }
    });
...
archivedRouter.get('/', roleCheck('admin'), async (req, res) => {
  try {
      ...
      include: {
        applicant: {
          select: {
            ...
            educationType: true, grade: true, createdAt: true
          }
        },
        groupStudents: {
    ...
    const archived = allStudents.filter(s =>
      ...
      s.groupStudents.every(gs => gs.status === 'archived')
    );

    const result = archived.map(s => {
      const lastGroupStudent = s.groupStudents
        .sort((a, b) => new Date(b.joinedAt) - new Date(a.joinedAt))[0];
      const archivedDate = lastGroupStudent?.joinedAt || s.applicant.createdAt;
      const lastPayment = s.payments[0] || null;
      const totalPaid = s.payments.reduce((sum, p) => sum + BigInt(p.amount), BigInt(0));
      const groups = s.groupStudents.map(gs => ({
        groupName: gs.group.name,
        subjectName: gs.group.subject.name,
        status: gs.status,
      ...
      return {
        ...
        phoneSelf: s.applicant.phoneSelf,
        phoneFather: s.applicant.phoneFather,
        phoneMother: s.applicant.phoneMother,
        educationType: s.applicant.educationType,
        grade: s.applicant.grade,
        registeredAt: s.applicant.createdAt,
        archivedDate,
        groups,
        lastPayment: lastPayment ? {
          amount: lastPayment.amount.toString(),
          monthYear: lastPayment.monthYear,
          paymentType: lastPayment.paymentType,
          groupName: lastPayment.group.name,
          subjectName: lastPayment.group.subject.name,
    ...
    result.sort((a, b) => new Date(b.archivedDate) - new Date(a.archivedDate));
    res.json(result);
  } catch (e) { res.status(500).json({ error: e.message }); }
});

module.exports = { attRouter, payRouter, expRouter, convRouter, balRouter, statRouter, debtRouter, resRouter, archivedRouter };
